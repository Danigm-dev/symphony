defmodule SymphonyElixir.Orchestrator do
  ...
  defp maybe_dispatch(%State{} = state) do
    ...
    with :ok <- Config.validate!(),
      ...
      choose_issues(normalize_issues(issues), state)
    else
      ...
      {:error, :missing_azure_devops_endpoint} ->
        Logger.error("Azure DevOps endpoint missing in WORKFLOW.md")
        state

      {:error, :missing_azure_devops_api_token} ->
        Logger.error("Azure DevOps API token missing in WORKFLOW.md")
        state

      {:error, :missing_azure_devops_project} ->
        Logger.error("Azure DevOps project missing in WORKFLOW.md")
        state

      {:error, {:azure_devops_not_implemented, operation}} ->
        Logger.error("Azure DevOps adapter not implemented for #{operation}")
        state
  ...
  def reconcile_issue_states_for_test(issues, %State{} = state) when is_list(issues) do
    reconcile_running_issue_states(normalize_issues(issues), state, active_state_set(), terminal_state_set())
  end
  ...
  @doc false
  @spec should_dispatch_issue_for_test(Issue.t() | map(), term()) :: boolean()
  ...
  @doc false
  @spec revalidate_issue_for_dispatch_for_test(Issue.t() | map(), ([String.t()] -> term())) ::
  ...
  def sort_issues_for_dispatch_for_test(issues) when is_list(issues) do
    sort_issues_for_dispatch(normalize_issues(issues))
  end
